...
import { RefreshCw, Edit, Save, X, AlertCircle, CheckCircle, History } from "lucide-react"
...
export function ExchangeRateModal({ isOpen, onClose }: ExchangeRateModalProps) {
  const { rates, isLoading, error, lastFetched, fetchRates, updateRate, getCurrencyFlag, history } =
  ...
  const handleSave = (baseCurrency: string, targetCurrency: string) => {
    ...
    if (!isNaN(newRate) && newRate > 0) {
      updateRate(baseCurrency, targetCurrency, newRate, true, user?.email || "unknown")
      setEditingRate(null)
      setEditValue("")
    }
  }
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Recent changes to exchange rates</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  ...
                  {history.length === 0 ? (
                    ...
                    history.map((entry) => (
                      <TableRow key={entry.id}>
                        ...
                        <TableCell>{entry.old_rate?.toFixed(6) || "N/A"}</TableCell>
                        <TableCell>{entry.new_rate.toFixed(6)}</TableCell>
                        <TableCell>
                          <Badge variant={entry.change_type === "manual_update" ? "secondary" : "outline"}>
                            {entry.change_type.replace("_", " ")}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-sm text-gray-600">{entry.updated_by || "System"}</TableCell>
